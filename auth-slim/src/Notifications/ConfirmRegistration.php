...
class ConfirmRegistration implements NotificationInterface
{
    protected $from;
    protected $to;
    protected $link;
    ...
    public function sendMail()
    {
        ...
        return (new Mail)
            ->subject("Confirmation to register") // email subject
            ->from([$from_email => $from_name]) // email => full name
            ->to([$to_email => $to_name]) // email => full name
            ->view("confirm-registration.twig", ['link' => $this->link]); // this is relative at view_path options
    }
}